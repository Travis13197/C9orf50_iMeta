...
# Set output directory using relative path format
...
# Using relative path format
rdata_dir <- ".../.../Rdata/"

# Create a helper function to load data with error checking
load_data_safely <- function(file_name, dir = rdata_dir) {
  file_path <- file.path(dir, file_name)
  if (file.exists(file_path)) {
    load(file_path, envir = .GlobalEnv)
    message(paste("Successfully loaded:", file_name))
    return(TRUE)
  } else {
    warning(paste("File not found:", file_path))
    return(FALSE)
...
load_data_safely("step2.deg.rdata")
load_data_safely("step3.3.GSEA_KK.rdata")
load_data_safely("step2.volcanoplot.rdata")
load_data_safely("step1.oridata_dedup_counts.rdata")
load_data_safely("step3.idconv_DEGs_genelist.rdata")

# Load pathview if needed
library(pathview) if (requireNamespace("pathview", quietly = TRUE))
...
if (exists("ISGsymbol") && length(ISGsymbol) > 0) {
  ...
  if (exists("genelist") && is.numeric(genelist)) {
    ...
    if (nrow(ISGgmt_entrezid) > 0) {
      ...
      ggsave(file.path(output_dir, "Fig5B_ISG_GSEA.pdf"), 
             plot = fig5b,
             height = 4.6, width = 4.2, dpi = 600)
      ggsave(file.path(output_dir, "Fig5B_ISG_GSEA.svg"), 
             plot = fig5b,
             height = 4.6, width = 4.2, dpi = 600)
      ggsave(file.path(output_dir, "Fig5B_ISG_GSEA.jpg"), 
             plot = fig5b,
             height = 4.6, width = 4.2, dpi = 600)
...
if (exists("dat_expr") && exists("deg") && exists("ISGsymbol")) {
  ...
  if (length(cg4) > 0) {
    ...
    ggsave(file.path(output_dir, "Fig5C_ISG_Heatmap.pdf"), 
           plot = fig5c,
           height = 8, width = 6, dpi = 600)
    ggsave(file.path(output_dir, "Fig5C_ISG_Heatmap.svg"), 
           plot = fig5c,
           height = 8, width = 6, dpi = 600)
    ggsave(file.path(output_dir, "Fig5C_ISG_Heatmap.jpg"), 
           plot = fig5c,
           height = 8, width = 6, dpi = 600)
...
This analysis is part of the manuscript examining C9orf50's role in regulating interferon stimulated genes (ISGs) expression in immune cells. The findings presented in Figure 5B-C highlight the significant enrichment of ISGs and their differential expression patterns in C9orf50 knockout cells.
